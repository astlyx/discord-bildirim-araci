...
try:
    ...
    QMessageBox.critical(None, "Eksik Paket",
        "discord.py kurulu değil.\n\npip install discord.py")
    sys.exit(1)
...
class BotThread(QThread):
    ...
    def run(self):
        ...
        async def on_ready():
            ...
            me.sig_ch.emit(chs)
            me.sig_st.emit(f"🔔  {me._cli.user}  ·  bağlandı", "ok")
        ...
        async def on_message(msg: discord.Message):
            ...
            if not msg.content and not msg.attachments:
                me.sig_st.emit(
                    "⚠  Mesaj içeriği boş — Discord Dev Portal'dan Message Content Intent'i kontrol et."
                    "Message Content Intent aktif edilmeli.", "warn")
                return
        ...
        try:
            ...
            me.sig_st.emit("☠  Geçersiz token — Ayarları kontrol et", "err")
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("🔔  Discord Bildirim Aracı")
    ...
    def _make_sidebar(self) -> QWidget:
        ...
        txt = QLabel("Bildirim Aracı"); txt.setObjectName("logo_txt")
        ...
        self.lbl_st = QLabel("● Bağlı değil")
    ...
    def _pg_dashboard(self) -> QWidget:
        ...
        self.sc_ch  = StatCard("0", "İZLENEN KANAL")
        self.sc_kw  = StatCard("0", "KELİME")
        self.sc_hit = StatCard("0", "ALGILAMA")
        for sc in (self.sc_ch, self.sc_kw, self.sc_hit): sr.addWidget(sc)
        vl.addLayout(sr)

        # Bot activity indicator
        self._lbl_activity = QLabel("○  Mesaj bekleniyor…")
        ...
        # Recent activity
        rl = QLabel("· · ·  SON AKTİVİTE  · · ·"); rl.setObjectName("section_lbl")
        ...
        empty_lbl = QLabel("Henüz algılama yok.")
    ...
    def _pg_channels(self) -> QWidget:
        ...
        top = QHBoxLayout()
        top.addWidget(self._title("⛩  KANALLAR")); top.addStretch()
        bs = QPushButton("✓  Seçimi Kaydet"); bs.setObjectName("btn_primary")
        bs.setFixedHeight(38); bs.clicked.connect(self._ch_save)
        top.addWidget(bs); vl.addLayout(top)
        vl.addWidget(self._sep())

        self._ch_info = QLabel(
            "🔔  Kanalları listelemek için Dashboard'dan botu başlatın."
            "Bot bağlandıktan sonra sunucunuzdaki tüm metin kanalları burada görünecek."
        )
    ...
    def _pg_keywords(self) -> QWidget:
        ...
        vl.addWidget(self._title("✦  KELİMELER"))
        vl.addWidget(self._sep())

        ir = QHBoxLayout(); ir.setSpacing(8)
        self._kw_inp = QLineEdit()
        self._kw_inp.setPlaceholderText("Yeni kelime veya ifade ekle…")
        ...
        vl.addWidget(self._slbl("· · ·  AKTİF KELİMELER  · · ·"))
        ...
        self._kw_empty = QLabel("Henüz kelime eklenmedi.")
    ...
    def _pg_settings(self) -> QWidget:
        ...
        vl.addWidget(self._title("⚙  AYARLAR"))
        vl.addWidget(self._sep())
        ...
        self._tok = QLineEdit()
        self._tok.setPlaceholderText("Bot tokenini buraya yapıştır")
        ...
        tok_hint = QLabel("→  discord.com/developers  ·  Bot sekmesi  ·  Token kopyala")
        ...
        # ntfy
        vl.addWidget(self._sep())
        vl.addWidget(self._slbl("· · ·  NTFY BİLDİRİM URL  · · ·"))

        self._ntfy_url = QLineEdit()
        self._ntfy_url.setPlaceholderText("https://ntfy.sh/konu-adın")
        self._ntfy_url.setFixedHeight(42); vl.addWidget(self._ntfy_url)

        hint = QLabel(
            "📱  Telefonuna ntfy uygulamasını kur ve aynı URL'e abone ol."
            "  ·  Örnek: https://ntfy.sh/benim-kanalım-xyz"
        )
        ...
        sv = QPushButton("⚔  Kaydet ve Uygula"); sv.setObjectName("btn_primary")
        sv.setFixedHeight(44); sv.clicked.connect(self._settings_save)
        tt = QPushButton("🔔  Test Bildirimi Gönder"); tt.setObjectName("btn_secondary")
    ...
    def _pg_logs(self) -> QWidget:
        ...
        top = QHBoxLayout()
        top.addWidget(self._title("☽  KAYITLAR")); top.addStretch()
        cl = QPushButton("✕  Tümünü Temizle"); cl.setObjectName("btn_danger")
        ...
        self._log_empty = QLabel("Henüz kayıt yok, botu başlatıp kanallar ve kelimeler seçtikten sonra algılamalar burada görünecek.")
    ...
    def _bot_start(self):
        ...
        if not tok:
            self._goto(3)
            self._set_status("⚠  Token girilmedi", "warn"); return
        ...
        self._bot.start()
        self._set_status("⏳  Bağlanıyor…", "warn")
    ...
    def _after_stop(self):
        self._set_status("● Bağlı değil", "idle")
    ...
    def _on_activity(self, info: str):
        self._lbl_activity.setText(f"●  İzlenen kanalda mesaj: {info}")
        self._lbl_activity.setStyleSheet(
    ...
    def _ch_save(self):
        ...
        self._update_stats()
        self._set_status(f"✓  {len(watched)} kanal kaydedildi", "ok")
    ...
    def _settings_save(self):
        ...
        if self._bot and self._bot.isRunning():
            ...
            self._set_status("✓  Ayarlar kaydedildi", "ok")

    def _settings_test(self):
        url = self._ntfy_url.text().strip()
        if not url:
            self._set_status("⚠  ntfy URL boş — bir URL gir", "warn")
            return
        try:
            r = requests.post(
                url,
                data="Discord Bildirim Aracı Çalışır Durumda!".encode("utf-8"),
                headers={
                    "Title":    "Bildirim",
                    "Priority": "4",
                    "Tags":     "bell",
            ...
            if r.ok:
                self._set_status("✓  Test bildirimi gönderildi!", "ok")
            else:
                self._set_status(f"✕  Sunucu yanıtı: HTTP {r.status_code} — URL'i kontrol et", "err")
        except requests.exceptions.ConnectionError:
            self._set_status("✕  Bağlantı kurulamadı — internet bağlantısını kontrol et", "err")
        except requests.exceptions.Timeout:
            self._set_status("✕  Zaman aşımı (8s) — sunucu yanıt vermiyor", "err")
        except Exception as e:
