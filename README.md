...
> A desktop GUI application that monitors Discord channels for keywords and sends push notifications via [ntfy.sh](https://ntfy.sh).
...
- A Discord bot token ([How to get one](#️-setup))
...
git clone https://github.com/YOUR_USERNAME/discord-bildirim-araci.git
...
Open the application → go to **Ayarlar (Settings)** and enter:
- Your **Discord Bot Token**
- Your **ntfy URL** (e.g. `https://ntfy.sh/my-discord-alerts-xyz`)

Click **Kaydet ve Uygula** to save.

---

## 🚀 Usage

1. **Start the bot** — Click **Botu Başlat** on the Dashboard
2. **Select channels** — Go to **Kanallar** and tick the channels to monitor, then click **Seçimi Kaydet**
3. **Add keywords** — Go to **Kelimeler** and type the words/phrases you want to be notified about
4. **Receive alerts** — Whenever a monitored channel contains a keyword, you get a push notification on your phone

The **Kayıtlar (Logs)** page keeps a history of all detections with timestamps, server name, channel, author and message preview.

---

## 📦 Building as EXE

> Produces a standalone `dist/DiscordBildirimAraci.exe` — no Python needed to run it.
